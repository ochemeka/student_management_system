...
$news = get_newsletter($_GET['nws']);
$news_part = mysqli_fetch_array($news);
...
$db_images = "";
			...
			if (empty($errors)) {
				// Perform Inssert
				//$passport = $db_images;
				$email = stripslashes($_REQUEST['name']);
...
?>
	...
	<?php if (!empty($message)) {
			...
			?>							
						...
						<form class="m-t-20" method="post" action="manage_editnewsletter.php?nws=<?php echo $_GET["nws"]; ?>" enctype="multipart/form-data">		
								 <!-- <input type="hidden" class="form-control"  value="<?php //echo time(); ?>" name="time">-->
								 
                                    <div class="">
                                        <div class="form-group">
										<span>Name <em style="color:#FF0000;"><?php echo $news_part['email']; ?></em></span>
                                            <input type="text" class="form-control" name="name" value="<?php echo $news_part['email']; ?>" name="email" placeholder="">
                                        </div>
